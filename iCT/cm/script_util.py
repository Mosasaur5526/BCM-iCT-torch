...
def create_ema_and_scales_fn(
    ...
    def ema_and_scales_fn(step):
        ...
        elif target_ema_mode == "adaptive" and scale_mode == "v1":
            scales = np.ceil(
                np.sqrt(
                    (step / total_steps) * ((end_scales + 1) ** 2 - start_scales**2)
                    + start_scales**2
        ...
        elif target_ema_mode == 'adaptive' and scale_mode == "exponential":
            rho = -1.0
            alpha1 = 1 / (start_scales + 1)
            alpha2 = 1 / (end_scales + 1)
            scales = alpha1 ** rho + step / total_steps * (alpha2 ** rho - alpha1 ** rho)
            scales = (np.floor(1 / scales ** (1 / rho)) + 1).astype(np.int32)
            target_ema = 0.
