...
def main():
    ...
    if args.bcf and not args.resume_checkpoint:
        print(f"Loading pretrained model from {args.pretrained_model_path} for BCF...")
        pretrained_state_dict = dist_util.load_state_dict(args.pretrained_model_path, map_location="cpu")
        model.load_state_dict(
            pretrained_state_dict,
            strict=False
        )

        model.time_embed_end = copy.deepcopy(model.time_embed)  # initialize t_end embedding layer from t embedding
        # initialize linear network to [I; 0], to make sure this initialization does not change the CM results
        embed_dim = model.time_embed_align.weight.data.shape[0]  # Note! weight in nn.Linear is saved in its transpose 
        model.time_embed_align.weight.data[:, : embed_dim] = torch.diag(torch.ones(embed_dim, dtype=model.time_embed_align.weight.dtype))
        model.time_embed_align.weight.data[:, embed_dim:] = torch.zeros((embed_dim, embed_dim), dtype=model.time_embed_align.weight.dtype)
