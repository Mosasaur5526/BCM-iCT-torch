...
class KarrasDenoiser:
    ...
    def get_scalings_for_bcm(self, sigma, sigma_end):
        c_skip = (sigma * sigma_end + self.sigma_data ** 2) / (sigma ** 2 + self.sigma_data ** 2)
        c_out = (sigma - sigma_end) * self.sigma_data / (sigma ** 2 + self.sigma_data ** 2) ** 0.5
        c_in = 1 / (sigma ** 2 + self.sigma_data ** 2) ** 0.5
        return c_skip, c_out, c_in
    ...
    def consistency_losses(
        ...
        def denoise_fn(x, t, t_end=None):
            return self.denoise(model, x, t, t_end, **model_kwargs)[1]

        if target_model:
            def target_denoise_fn(x, t, t_end=None):
                # ADD these lines for sanity check
                for param in target_model.parameters():
                    assert param.requires_grad == False
                return self.denoise(target_model, x, t, t_end, **model_kwargs)[1]
            # @th.no_grad()
            # def target_denoise_fn(x, t, t_end=None):
            #     return self.denoise(target_model, x, t, t_end, **model_kwargs)[1]
        ...
        P2[num_scales - 2] = 0.
        P2 = P2 / np.sum(P2)
        indices = np.random.choice(max_n_scales - 1, (x_start.shape[0],), True, P2)
        ...
        elif self.loss_norm == "ict":  # V2, sec 3.3 PSEUDO-HUBER METRIC FUNCTIONS
            c = 0.06
        ...
        if not ct_loss_only:
            ...
            for i in range(len(t3_indices)):
                P3 = np.where(np.arange(max_n_scales - 1) != indices[i] + 1,
                              P,
                              np.zeros(max_n_scales - 1))
                P3 = P3 / np.sum(P3)
                t3_indices[i] = np.random.choice(max_n_scales - 1, (), True, P3)
            t3 = th.from_numpy(ts[t3_indices]).to(x_start.device).float()

            th.manual_seed(seed)
            est_t3 = denoise_fn(x_t, t, t3)

            th.manual_seed(seed)
            distiller_t3 = target_denoise_fn(est_t3, t3)
            losses_soft = (distiller_t3 - distiller_target) ** 2
            losses_soft = losses_soft.reshape(losses_soft.shape[0], -1)
            data_dim = losses_soft.shape[-1]
            losses_soft = th.sqrt(th.sum(losses_soft, axis=-1) + c ** 2) - c
            losses_soft = losses_soft / np.sqrt(data_dim)
            weights_soft = 1 / (th.abs(t3 - t) + 1e-20)
            loss_soft = losses_soft * weights_soft
        ...
        if ct_loss_only:
            ...
            terms["loss_ct"] = loss
            terms["loss_soft"] = loss_soft
            terms["loss"] = loss + loss_soft
    ...
    def denoise(self, model, x_t, sigmas, sigmas_end=None, **model_kwargs):
        import torch.distributed as dist
        if sigmas_end is None:
            sigmas_end = torch.ones_like(sigmas) * self.sigma_min  # 新加的

        c_skip, c_out, c_in = [
            append_dims(x, x_t.ndim)
            for x in self.get_scalings_for_bcm(sigmas, sigmas_end)
        ]

        rescaled_t = 1000 * 0.25 * th.log(sigmas + 1e-44)
        rescaled_t_end = 1000 * 0.25 * th.log(sigmas_end + 1e-44)
        model_output = model(c_in * x_t, rescaled_t, rescaled_t_end, **model_kwargs)
