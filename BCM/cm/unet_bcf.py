...
class QKVAttentionLegacy(nn.Module):
    ...
    def __init__(self, n_heads, cosine=True):
        super().__init__()
    ...
    def forward(self, qkv):
        ...
        if self.cosine:
            q = q.reshape(bs, -1, ch, 1, length)
            k = k.reshape(bs, -1, ch, 1, length)
            v = v.reshape(bs, -1, ch, 1, length)
            q, k, v = normalize(th.cat([q, k, v], -2), dim=2).unbind(3)
            # q, k = normalize(th.cat([q, k], -2), dim=2).unbind(3)
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.dropout = dropout
        self.dropout_highres = dropout_highres
        ...
        self.time_embed_end = nn.Sequential(
            linear(model_channels, time_embed_dim),
            nn.SiLU(),
            linear(time_embed_dim, time_embed_dim),
        )

        self.time_embed_align = linear(time_embed_dim * 2, time_embed_dim, bias=False) 
        ...
        for level, mult in enumerate(channel_mult):
            # level 0: 64, dropout = 0
            # level 1: 32, dropout = 0
            # level 2: 16, dropout = 0.2
            # level 3: 8, dropout = 0.2
            ##########
            if level > 1:
                _dropout = self.dropout_highres
            else:
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            # level 0: 8, dropout = 0.2
            # level 1: 16, dropout = 0.2
            # level 2: 32, dropout = 0
            # level 3: 64, dropout = 0
            ##########
            if level >= len(list(enumerate(channel_mult))) - 2:
                _dropout = self.dropout_highres
            else:
                _dropout = self.dropout
            ##########
    ...
    def forward(self, x, timesteps, timesteps_end, y=None):
        """
        ...
        hs = []
        # emb = th.cat([timestep_embedding(timesteps, self.model_channels),
        #               timestep_embedding(timesteps_end, self.model_channels)], -1)
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        emd_end = self.time_embed_end(timestep_embedding(timesteps_end, self.model_channels))
        emb = self.time_embed_align(th.cat([emb, emd_end], dim=-1))
